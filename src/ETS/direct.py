...
def run_experiment(CONFIG):
    ...
    if CONFIG["model"]["HWS"]:
        ...
        seasonal_pattern = W_decomp.seasonal.groupby(
            W_decomp.seasonal.index.dayofweek
        ).mean()
    ...
    if CONFIG["forecast"]["resolution"] == "hourly":
        # HOURLY SEASONALITY
        h_decomp = seasonal_decompose(y_hourly, model="additive", period=24)
        # unused variable - ignored by ruff (noqa) in development
        seasonal_hourly = h_decomp.seasonal  # noqa
