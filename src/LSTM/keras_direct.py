...
def run_experiment(CONFIG):
    ...
    (
        ...
        train_df, val_df, test_df, k=lags[-1], resolution=CONFIG["data"]["resolution"]
    )
    ...
    # predictions
    preds = model.predict(
        [X_past_test, X_future_test], batch_size=CONFIG["training"]["batch_size"]
    ).squeeze()
    preds = scaler.inverse_transform(
        np.hstack(
            [preds.reshape(-1, 1), np.zeros((len(preds), scaler.n_features_in_ - 1))]
        )
    )[:, 0]
    targets = scaler.inverse_transform(
        np.hstack(
            [y_test.reshape(-1, 1), np.zeros((len(y_test), scaler.n_features_in_ - 1))]
        )
    )[:, 0]
