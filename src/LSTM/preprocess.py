...
def clean_raw_data(data_dir):
    ...
    household_data_path = os.path.join(
        data_dir, "opsd-household_data-2020-04-15/household_data_60min_singleindex.csv"
    )
    weather_data_path = os.path.join(
        data_dir, "opsd-weather_data-2020-09-16/weather_data.csv"
    )
    ...
    household_data.columns = household_data.columns.str.replace(
        "DE_KN_", "", regex=False
    )
    ...
    residential4_weather.columns = residential4_weather.columns.str.replace(
        "DE_", "", regex=False
    )
...
def engineer_features(df_hourly, df_daily):
    ...
    hourly["daylight_flag"] = hourly.index.to_series().apply(
        lambda x: dawn(city.observer, date=x.date())
        <= x
        <= dusk(city.observer, date=x.date())
    )
...
def run_data_pipeline(raw_data_dir, output_data_dir):
    ...
    hourly_train.to_csv(
        os.path.join(output_data_dir, "residential4_energy_demand_hourly_train.csv")
    )
    hourly_test.to_csv(
        os.path.join(output_data_dir, "residential4_energy_demand_hourly_test.csv")
    )

    daily_all.to_csv(
        os.path.join(output_data_dir, "residential4_energy_demand_daily.csv")
    )
    daily_train.to_csv(
        os.path.join(output_data_dir, "residential4_energy_demand_daily_train.csv")
    )
    daily_test.to_csv(
