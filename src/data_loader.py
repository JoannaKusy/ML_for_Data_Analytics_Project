...
def loader():
    ...
    X["datetime"] = pd.to_datetime(
        X["Date"] + " " + X["Time"], dayfirst=True
    )  # dayfirst= True because the format of the date is DD/MM/YYYY not MM/DD/YYYY
...
def data_loader():
    ...
    X.replace(
        "?", np.nan, inplace=True
    )  # missing value is replace by "?" right now i changed it by Nan but need to use the function interpolate later
    X = X.astype(float)
