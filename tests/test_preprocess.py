...
from LSTM.preprocess import clean_raw_data, run_data_pipeline, encode_features
...
def test_clean_raw_data(monkeypatch):
    ...
    assert (
        list(hourly_df.columns) == expected_columns
    ), "Hourly columns do not match expected output"

    # expect 47 rows because diff() makes the first row NaN, which is then dropped.
    assert (
        hourly_df.shape[0] == 47
    ), f"Hourly dataset should have exactly 47 rows, but has {hourly_df.shape[0]}"

    assert (
        "cet_cest_timestamp" not in hourly_df.columns
    ), "Timezone column was not dropped"
    ...
    if daily_df.shape[0] == 1:
        assert daily_df.index[0] == pd.Timestamp(
            "2015-10-15 00:00:00", tz="UTC"
        ), "Remaining day should be the 15th"
    ...
    assert (
        abs(daily_df.loc["2015-10-15", "grid_import"] - 60.5) < 1e-6
    ), "Diff calculation failed for residential data"

    # Check the weather data (ensure it was NOT diffed)
    # Day 2 temp should just be the mean of day 2 (20.0), NOT 20 - 10 = 10.
    assert (
        daily_df.loc["2015-10-15", "temperature"] == 20.0
    ), "Weather data was incorrectly diffed instead of averaged"
...
def test_run_data_pipeline(monkeypatch, tmp_path):
    ...
    # ('winter' for Dec/Jan)
    assert (
        daily_all.loc["2017-01-01", "season"] == "winter"
    ), "Season feature mapping failed"
    ...
    df_hourly_train = pd.read_csv(
        output_dir / "residential4_energy_demand_hourly_train.csv", index_col=0
    )
    df_hourly_test = pd.read_csv(
        output_dir / "residential4_energy_demand_hourly_test.csv", index_col=0
    )

    assert (
        df_hourly_train.shape[0] == 47
    ), "Hourly Train dataset size incorrect (Should only be 2016)"
    assert (
        df_hourly_test.shape[0] == 48
    ), "Hourly Test dataset size incorrect (Should only be 2017)"

    # Dec 30th dropped by diff)
    # 2016: Dec 31 (1 day)
    # 2017: Jan 1 + Jan 2 (2 days)
    df_daily_train = pd.read_csv(
        output_dir / "residential4_energy_demand_daily_train.csv", index_col=0
    )
    df_daily_test = pd.read_csv(
        output_dir / "residential4_energy_demand_daily_test.csv", index_col=0
    )
...
def test_encode_features():
    ...
    test_data = pd.DataFrame(
        {"energy_demand": [130], "is_holiday_or_weekend": [True], "season": ["spring"]}
    )

    train_encoded, test_encoded = encode_features(
        train_data, test_data, resolution="daily"
    )
